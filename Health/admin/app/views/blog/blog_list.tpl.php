...
<div class="page-title">
	<div class="row align-items-center">
		...
		<div class="col-sm-6 text-right">
			...
			<?php if ($page_add) { ?>
				<a href="<?php echo URL_ADMIN.DIR_ROUTE.'blog/add'; ?>" class="btn btn-primary btn-sm"><i class="ti-plus pr-2"></i> New Blog</a>
			<?php } ?>
		</div>
	</div>
</div>
				...
				<tbody>
					<?php if (!empty($result)) { foreach ($result as $key => $value) { ?>
						<tr>
							...
							<td><a href="<?php echo URL_ADMIN.DIR_ROUTE.'user/edit&id='.$value['user_id'] ?>" target="_blank"><?php echo $value['name']; ?></a></td>
							<td><?php echo date_format(date_create($value['date_of_joining']), $common['info']['date_format']); ?></td>
							<?php if ($page_delete || $page_edit) { ?>
								<td class="table-action">
									<?php if ($page_edit) { ?>
										<a href="<?php echo URL_ADMIN.DIR_ROUTE.'blog/edit&id='.$value['id'];?>" class="text-primary edit" data-toggle="tooltip" title="Edit"><i class="ti-pencil-alt"></i></a>
									<?php } if ($page_delete) { ?>
