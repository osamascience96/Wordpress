...
<div class="page-title">
    <div class="row align-items-center">
        ...
        <div class="col-sm-6 text-right">
            ...
            <?php if ($page_add) { ?>
                <a href="<?php echo URL_ADMIN.DIR_ROUTE.'birthrecord/add'; ?>" class="btn btn-primary btn-sm"><i class="ti-plus pr-2"></i> Add Birth Record</a>
            <?php } ?>
        </div>
    </div>
</div>
                ...
                <tbody>
                    <?php if ($result) { foreach ($result as $key => $value) { ?>
                        <tr> 
                            ...
                            <td><a class="text-primary"><?php echo $value['child']; ?></a></td>
                            <td><?php echo $value['gender']; ?></td>
                            <td><?php echo date_format(date_create($value['date']), $common['info']['date_format']).' '.$value['time']; ?></td>
                                ...
                                <td class="table-action">
                                    <?php if ($page_edit || $page_view) { ?>
                                        <div class="dropdown d-inline-block">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-right export-button">
                                                <?php if ($page_view) { ?>
                                                    <li><a href="<?php echo URL_ADMIN.DIR_ROUTE.'birthrecord/view&id='.$value['id'];?>"><i class="ti-layout-media-center-alt pr-2"></i>View</a></li>
                                                <?php } if ($page_edit) { ?>
                                                    <li><a href="<?php echo URL_ADMIN.DIR_ROUTE.'birthrecord/edit&id='.$value['id'];?>"><i class="ti-pencil-alt pr-2"></i>Edit</a></li>
                                                <?php } ?>
                                            </ul>
                                        </div>
                                    <?php } if ($page_delete) { ?>
