...
<div class="page-title">
    <div class="row align-items-center">
        <div class="col-sm-6">
            <h2 class="page-title-text d-inline-block">Pages</h2>
        ...
        <div class="col-sm-6 text-right">
            ...
            <?php if ($page_add) { ?>
                <a href="<?php echo URL_ADMIN.DIR_ROUTE.'page/add'; ?>" class="btn btn-primary btn-sm"><i class="ti-plus pr-2"></i> New Page</a>
            <?php } ?>
        </div>
    </div>
</div>
                ...
                <tbody>
                    <?php if (!empty($result)) { foreach ($result as $key => $value) { ?>
                        <tr>
                            ...
                            <td><a class="font-14 m-0 text-primary"><?php echo $value['page_title'];?></a></td>
                            <td><?php echo date_format(date_create($value['last_modified']), $common['info']['date_format']); ?></td>
                            <td><?php echo date_format(date_create($value['date_of_joining']), $common['info']['date_format']); ?></td>
                            <?php if ($page_edit || $page_delete) { ?>
                                <td class="table-action">
                                    <?php if ($page_edit) { ?>
                                        <a href="<?php echo URL_ADMIN.DIR_ROUTE.'page/edit&id='.$value['id'];?>" class="text-primary edit" data-toggle="tooltip" title="Edit"><i class="ti-pencil-alt"></i></a>
                                    <?php } if ($page_delete && $value['predefined'] == '0') { ?>
